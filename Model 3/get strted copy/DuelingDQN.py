...
class DuelingDQN(nn.Module):
    def __init__(self, state_size, action_size):
        ...
        # New fully connected layers with increased neurons and additional layer
        self.fc1 = nn.Linear(input_size, 512)  # Increased from 256 to 512
        self.fc2 = nn.Linear(512, 256)         # New layer with 256 neurons
        self.fc3 = nn.Linear(256, 128)         # Increased from 128 to 256
        
        # Value stream with increased neurons
        self.value_stream = nn.Sequential(
            nn.Linear(128, 96),                 # Increased from 64 to 96
            nn.ReLU(),
            nn.Linear(96, 1)                    # Increased from 64 to 96
        )
        
        # Advantage stream with increased neurons
        self.advantage_stream = nn.Sequential(
            nn.Linear(128, 96),                 # Increased from 64 to 96
            nn.ReLU(),
            nn.Linear(96, action_size)          # Increased from 64 to 96
        )
    ...
    def forward(self, x):
        ...
        x = F.relu(self.fc3(x))  # Previously fc2, now fc3
